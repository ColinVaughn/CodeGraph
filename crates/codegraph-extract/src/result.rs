...
// The extraction-fact contract types live in `codegraph-core` so the graph crate
// can consume them without depending on `extract`. Re-exported here for callers
// that reach for `codegraph_extract::{RawCall, ImportRecord}`.
pub use codegraph_core::{ImportRecord, RawCall};

/// The result of extracting a single file: graph fragments plus unresolved
/// calls and import evidence. `nodes`/`edges` feed `codegraph-graph::build`;
