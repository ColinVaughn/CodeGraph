//! Filesystem-watch support: the pure debounce/filter cores behind `codegraph
...
mod tests {
    ...
    fn ignores_output_vcs_and_build_subtrees() {
        for p in [
            ...
            "proj/codegraph-out/cache/manifest.json",
            ".git/index",
            "a/.git/HEAD",
            "target/debug/foo",
            "node_modules/x/y.js",
    ...
    fn batch_dedups_and_skips_ignored() {
        ...
        assert!(
            !b.record(PathBuf::from("codegraph-out/graph.json")),
