//! Incremental rebuild + git integration for CodeGraph.
//!
...
//! `graph.json` and the other artifacts. `affected` lives in `codegraph-query`
...
use codegraph_detect::{classify_file, detect, FileType};
...
///
/// Mirrors `codegraph extract`'s assembly (build → resolve → dedup → cluster)
...
pub fn rebuild(
    ...
    // and keeps rel ids/source_files identical to a full `codegraph extract`).
    let root = opts
        .root
        .canonicalize()
    ...
    // (update/watch/workspace), matching `codegraph extract`. (.NET/apex/etc. are
    ...
    let cache_dir = root.join("codegraph-out").join("cache");
    ...
    let aliases = codegraph_extract::load_alias_resolver(root, &det.ts_config_files);
    codegraph_extract::resolve_imports(&mut nodes, &mut edges, &aliases);
...
mod tests {
    use super::*;
    use codegraph_core::{Confidence, FileType as CoreFt};
