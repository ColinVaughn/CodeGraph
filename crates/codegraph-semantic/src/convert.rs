...
//! `codegraph-core` nodes/edges. Concept nodes are tagged `_origin: "semantic"`
...
mod tests {
    ...
    fn semantic_concepts_flow_through_dedup_fuzzy_merge() {
        // Converted LLM concept output runs through codegraph-graph's fuzzy dedup
        // pass (a no-op on code-only graphs); near-identical concepts collapse to one.
        use codegraph_graph::deduplicate_entities;
