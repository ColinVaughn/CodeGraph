...
//! file exploration, only when `codegraph-out/graph.json` exists:
...
//! is idempotent: any prior CodeGraph hook (matched by matcher + the literal
//! `codegraph` in its body) is removed before the current pair is appended, so
...
/// Matchers a CodeGraph PreToolUse hook may use (current + legacy), used to
...
const BASH_HOOK_COMMAND: &str = r#"CMD=$(python3 -c "import json,sys; d=json.load(sys.stdin); print(d.get('tool_input',d).get('command',''))" 2>/dev/null || true); case "$CMD" in *grep*|*rg\ *|*ripgrep*|*find\ *|*fd\ *|*ack\ *|*ag\ *)   [ -f codegraph-out/graph.json ] &&   echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","additionalContext":"MANDATORY: codegraph-out/graph.json exists. You MUST run `codegraph query \"<question>\"` before grepping raw files. Only grep after codegraph has oriented you, or to modify/debug specific lines."}}'   || true ;; esac"#;

/// The Read|Glob hook: nudge before reading a source/doc file outside the graph dir.
const READ_HOOK_COMMAND: &str = r#"HIT=$(python3 -c "import json,sys;d=json.load(sys.stdin);t=d.get('tool_input',d);s=(str(t.get('file_path') or '')+' '+str(t.get('pattern') or '')+' '+str(t.get('path') or '')).lower().replace(chr(92),'/');exts=('.py','.js','.ts','.tsx','.jsx','.go','.rs','.java','.rb','.c','.h','.cpp','.hpp','.cc','.cs','.kt','.swift','.php','.scala','.lua','.sh','.md','.rst','.txt','.mdx');sys.stdout.write('1' if 'codegraph-out/' not in s and any(e in s for e in exts) else '')" 2>/dev/null || true); if [ "$HIT" = 1 ] && [ -f codegraph-out/graph.json ]; then echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","additionalContext":"MANDATORY: codegraph-out/graph.json exists. You MUST run codegraph before reading source files. Use: `codegraph query \"<question>\"` (scoped subgraph), `codegraph explain \"<concept>\"`, or `codegraph path \"<A>\" \"<B>\"`. Only read raw files after codegraph has oriented you, or to modify/debug specific lines. This rule applies to subagents too — include it in every subagent prompt involving code exploration."}}'; fi || true"#;
...
/// `codegraph` somewhere in its serialized body.
fn is_codegraph_hook(hook: &Value) -> bool {
    let matcher_ok = hook
        .get("matcher")
        .and_then(Value::as_str)
        .map(|m| HOOK_MATCHERS.contains(&m))
        .unwrap_or(false);
    matcher_ok && hook.to_string().contains("codegraph")
}
...
/// The current `PreToolUse` array with any CodeGraph entries removed.
...
/// Install (or refresh) the CodeGraph PreToolUse hooks in `.claude/settings.json`,
...
pub fn install_settings_hook(repo_root: &Path) -> std::io::Result<PathBuf> {
    ...
    pre.extend(codegraph_hooks());
    set_pretooluse(&mut settings, pre);
    write_settings(&path, &settings)?;
    Ok(path)
}

/// Remove the CodeGraph PreToolUse hooks. No-op if the file is missing or holds
...
mod tests {
    ...
    fn install_writes_two_hooks_preserving_foreign_content() {
        ...
        assert!(pre
            .iter()
            .any(|h| h["matcher"] == "Bash" && is_codegraph_hook(h)));
        assert!(pre
            .iter()
            .any(|h| h["matcher"] == "Read|Glob" && is_codegraph_hook(h)));
    ...
    fn install_is_idempotent() {
        ...
        install_settings_hook(root).unwrap();
        let pre = read_pre(root);
        let ours = pre.iter().filter(|h| is_codegraph_hook(h)).count();
    ...
    fn corrupt_settings_is_reset_not_crash() {
        ...
        install_settings_hook(root).unwrap();
        let pre = read_pre(root);
        assert_eq!(pre.iter().filter(|h| is_codegraph_hook(h)).count(), 2);
    }
    ...
    #[test]
    fn hook_commands_are_codegraph_branded() {
        // Guard against shipping wrong branding / wrong graph dir.
        assert!(BASH_HOOK_COMMAND.contains("codegraph-out/graph.json"));
        assert!(BASH_HOOK_COMMAND.contains("codegraph query"));
        assert!(READ_HOOK_COMMAND.contains("codegraph explain"));
    }
}
