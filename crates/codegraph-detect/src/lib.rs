//! CodeGraph file detection: discovery, classification, ignore handling, manifest.
#![forbid(unsafe_code)]
...
/// Discover and classify files under `root`. Honors `.codegraphignore` and
/// `.gitignore` (both apply, per-directory + parents; `.codegraphignore` wins on
...
pub fn detect(root: &Path) -> DetectResult {
    ...
    // Both `.codegraphignore` and `.gitignore` apply, layered per-directory and up
    // to the VCS root. `add_custom_ignore_filename` gives `.codegraphignore` higher
    ...
    // `.codegraphignore` existed, which silently dropped every subdir/parent
...
mod tests {
    ...
    #[test]
    fn codegraphignore_excludes_with_negation() {
        let dir = tempfile::tempdir().unwrap();
        write(dir.path(), ".codegraphignore", "*.py\n!keep.py\n");
    ...
    fn negation_cannot_rescue_under_excluded_dir() {
        ...
        write(dir.path(), ".codegraphignore", "android/\n!src/\n");
    ...
    #[test]
    fn gitignore_and_codegraphignore_layer_with_codegraphignore_precedence() {
        let dir = tempfile::tempdir().unwrap();
        fs::create_dir_all(dir.path().join(".git")).unwrap();
        // .gitignore excludes vendor/ AND keep.py; .codegraphignore excludes other.py
        // and RE-INCLUDES keep.py (precedence over .gitignore).
        write(dir.path(), ".gitignore", "vendor/\nkeep.py\n");
        write(dir.path(), ".codegraphignore", "other.py\n!keep.py\n");
        ...
        assert!(
            ...
            "codegraphignore applies"
        );
        assert!(
            names.contains(&"keep.py".to_string()),
            "codegraphignore !keep.py wins over .gitignore"
        );
    }

    #[test]
    fn subdir_gitignore_still_applies_under_root_codegraphignore() {
        // Audit regression: a root .codegraphignore must NOT disable a subdir's
        // .gitignore (the old toggle did, leaking ignored files in).
        let dir = tempfile::tempdir().unwrap();
        fs::create_dir_all(dir.path().join(".git")).unwrap();
        write(dir.path(), ".codegraphignore", "rootignored.py\n");
