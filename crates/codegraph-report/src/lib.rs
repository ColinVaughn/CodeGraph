...
use codegraph_graph::{cohesion_score, AnalysisResult, KnowledgeGraph};
...
fn confidence_str(c: codegraph_core::Confidence) -> &'static str {
    match c {
        codegraph_core::Confidence::Extracted => "EXTRACTED",
        codegraph_core::Confidence::Inferred => "INFERRED",
        codegraph_core::Confidence::Ambiguous => "AMBIGUOUS",
    }
}
...
mod tests {
    ...
    use codegraph_graph::{analyze, apply_communities, cluster, ClusterOptions};
    use serde_json::Map;
    ...
    fn report_writes_to_disk() {
        ...
        let p = dir.path().join("codegraph-out/GRAPH_REPORT.md");
