//! CodeGraph workspace federation.
//!
...
//! **global graph store** (id-prefixing + a `~/.codegraph` store; see [`global`])
...
pub enum WorkspaceError {
    ...
    /// A `codegraph-workspace.toml` (or member manifest) failed to parse.
    ...
    Member {
        member: String,
        source: codegraph_incremental::IncrementalError,
    },
