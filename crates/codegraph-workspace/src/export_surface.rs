...
use codegraph_core::{Confidence, FileType, GraphData, NodeId};
...
pub fn resolve_cross_repo(
    ...
    let mut deduped: Vec<codegraph_core::Edge> = Vec::with_capacity(g.links.len());
...
mod tests {
    ...
    fn non_import_edge_to_external_symbol_also_resolves() {
        ...
        let refs: Vec<&codegraph_core::Edge> = g
            .links
            .iter()
    ...
    fn cargo_coordinate_matches_underscore_import_path() {
        // Cargo package "codegraph-core" is imported as `use codegraph_core::{...}`
        // -> stub "codegraph_core". The `-` to `_` normalization must match it.
        let a = gd(
            vec![
                node("app", "app", "app.rs"),
                node("codegraph_core", "codegraph_core", ""),
            ],
            vec![import_edge("app", "codegraph_core", "imports_from")],
        );
        let b = gd(vec![node("Node", "Node", "node.rs")], vec![]);
        let composed = compose(vec![("repoa".into(), a), ("core".into(), b.clone())]);
        let surfaces = vec![build_export_surface(
            "core",
            coord("codegraph-core", Ecosystem::Cargo),
            &b,
        )];
        let (g, report) =
            resolve_cross_repo(composed, &surfaces, &crate::alias::AliasMap::default());
