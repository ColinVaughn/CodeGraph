...
//! `codegraph-out/workspace-state.json`. Source hashes let [`update_workspace`]
...
use codegraph_detect::{detect, Manifest, ALL_FILE_TYPES};
...
pub fn state_path(root: &Path) -> PathBuf {
    root.join("codegraph-out").join("workspace-state.json")
}
...
pub fn update_workspace(root: &Path, opts: &WorkspaceBuildOptions) -> Result<UpdateOutcome> {
    ...
    let changed_members: Vec<String> = statuses
        ...
        .map(|s| s.tag.clone())
        .collect();
    let graph_exists = root.join("codegraph-out").join("graph.json").is_file();
...
/// `--changed` run can short-circuit. Used by `codegraph workspace build`.
...
mod tests {
    ...
    fn first_update_builds_then_second_is_a_noop() {
        ...
        crate::write_graph(
            &d.path().join("codegraph-out").join("graph.json"),
