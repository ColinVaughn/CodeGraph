...
/// Expand explicit `codegraph-workspace.toml` member globs (relative to `root`)
...
pub fn discover_project_roots(root: &Path) -> Vec<Member> {
    ...
    for dir in codegraph_detect::walk_dirs(&canon, Some(FALLBACK_MAX_DEPTH)) {
    ...
    for sub in codegraph_detect::submodule_paths(&canon) {
