...
//! directory (default `~/.codegraph`, but injectable for tests via
...
use codegraph_incremental::union_graphs;
...
impl GlobalStore {
    ...
    /// The default store directory: `~/.codegraph` (`%USERPROFILE%\.codegraph`
    /// on Windows). Falls back to `.codegraph` in the CWD if no home is set.
    pub fn default_dir() -> PathBuf {
        let home = std::env::var_os("HOME")
            .or_else(|| std::env::var_os("USERPROFILE"))
            .map(PathBuf::from);
        match home {
            Some(h) => h.join(".codegraph"),
            None => PathBuf::from(".codegraph"),
        }
    }
...
mod tests {
    use super::*;
    use codegraph_core::{FileType, Node};
    use serde_json::Map;
