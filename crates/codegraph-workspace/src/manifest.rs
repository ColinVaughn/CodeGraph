//! The `codegraph-workspace.toml` model. Declares a workspace's
...
/// The parsed `codegraph-workspace.toml`.
...
pub const MANIFEST_NAME: &str = "codegraph-workspace.toml";

/// Load `root/codegraph-workspace.toml`. `Ok(None)` when the file is absent;
...
/// Write `manifest` to `root/codegraph-workspace.toml`.
