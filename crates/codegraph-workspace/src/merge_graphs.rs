//! `codegraph merge-graphs` — compose several `graph.json` files into one
//! namespaced graph.
//!
//! Each input's repo tag is derived from its grandparent directory name (so
//! `<repo>/codegraph-out/graph.json` → `<repo>`). Inputs are prefixed and
...
/// (`<repo>/codegraph-out/graph.json` → `repo`), falling back to the file stem,
...
mod tests {
    use super::*;
    use codegraph_core::{FileType, GraphData, Node, NodeId};
    use serde_json::Map;
    ...
    fn tag_is_the_grandparent_dir() {
        assert_eq!(
            tag_for(Path::new("/x/billing/codegraph-out/graph.json")),
            "billing"
        );
    ...
    fn merges_two_graphs_with_namespaced_ids() {
        ...
        let a = d.path().join("billing/codegraph-out/graph.json");
        let b = d.path().join("identity/codegraph-out/graph.json");
