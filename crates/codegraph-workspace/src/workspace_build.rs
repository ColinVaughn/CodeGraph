...
//! pipeline (one `codegraph_incremental::rebuild` call), publish each member's
...
/// An already-cloned member is reused as-is (`codegraph workspace sync` refreshes
...
fn fetch_subgraph(name: &str, url: &str) -> Result<GraphData> {
    let bytes = codegraph_ingest::safe_fetch(url, crate::MAX_GRAPH_BYTES).map_err(|e| {
...
/// Resolve a workspace's members: from `codegraph-workspace.toml` (member globs +
...
pub fn build_workspace(root: &Path, opts: &WorkspaceBuildOptions) -> Result<WorkspaceBuild> {
    ...
    let cache = root.join("codegraph-out").join("workspace-repos");
